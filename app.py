...
def write_review():
    ...
    if request.cookies.get('login') == 'registered':
        sql_insert = "INSERT INTO D0018E.Rating (RaID, RRating, RReview, PrID, ReID) VALUES({0}, {1}, {2}, (SELECT PID FROM D0018E.Product WHERE PID = {3}), (SELECT RID FROM D0018E.Registered WHERE RID = {4}));".format(RaID, request.form['RRating'], request.form['RReview'], int(data1), cookie_id)
        print(sql_insert)
        execute(sql_insert, False)
    elif request.cookies.get('login') == 'None':
